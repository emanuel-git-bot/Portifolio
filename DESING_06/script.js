...
(function () {
  ...
  /* ---------------------------------------------------------
     Lightbox das telas e vídeos de projeto — reaproveita o src que o
     próprio thumbnail já carregou (sem precisar de outro atributo/arquivo,
     já que o build-arquivo-unico.py só embute mídia referenciada pelo
     atributo src, não href). Anterior/Próximo navegam dentro da MESMA
     galeria (.work-gallery) em que o thumbnail clicado está, com loop nas
     pontas -- cada projeto tem sua própria lista independente.
     --------------------------------------------------------- */
  ...
  if (lightbox && lightboxImg && lightboxVideo) {
    ...
    var showAt = function (index) {
      ...
      var media = btn.querySelector('video, img');
      if (!media) return;
      lightboxVideo.pause();
      ...
      if (lightboxCounter) {
        lightboxCounter.textContent = currentGroup.length > 1
    ...
    document.querySelectorAll('.work-gallery__thumb').forEach(function (btn) {
      btn.addEventListener('click', function () {
        var gallery = btn.closest('.work-gallery');
        currentGroup = gallery ? Array.from(gallery.querySelectorAll('.work-gallery__thumb')) : [btn];
    ...
    if (lightboxPrev) lightboxPrev.addEventListener('click', function () { showAt(currentIndex - 1); });
    if (lightboxNext) lightboxNext.addEventListener('click', function () { showAt(currentIndex + 1); });
  ...
  if (cursorDot && !reduced && finePointer) {
    ...
    var cdInteractiveSelector = "a, button, [data-link], .what-content, .tile--icon, #snowman, .cta-globe canvas, .stack-3d canvas, .what-3d canvas";
  ...
  /* ---------------------------------------------------------
     Ferramentas: a luz do card segue o mouse e o card inclina um
     pouco na direção dele (via variáveis CSS no .tool-card__in)
     --------------------------------------------------------- */
  if (finePointer && !reduced) {
    document.querySelectorAll('.tool-card__in').forEach(function (card) {
  ...
  /* ---------------------------------------------------------
     Sala de Estudos: estante de livros. Só um livro aberto por vez —
     abre no hover, no foco do teclado e no primeiro toque (o segundo
     toque, já aberto, segue o link normalmente).
     --------------------------------------------------------- */
  var books = Array.prototype.slice.call(document.querySelectorAll('.book'));
  if (books.length) {
    var openBook = function (book) {
      books.forEach(function (b) { b.classList.toggle('is-open', b === book); });
    };
    books.forEach(function (book) {
      book.addEventListener('pointerenter', function (e) {
        if (e.pointerType === 'mouse') openBook(book);
      });
      book.addEventListener('focus', function () { openBook(book); });
