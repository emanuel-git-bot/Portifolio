...
Embute no HTML: CSS, JS, GSAP, ScrollTrigger, Lenis, a camada 3D (Three.js,
já compilada em assets/vendor/scene3d.min.js), as fontes Geist (woff2), as
imagens e os vídeos — tudo como data URI ou tag inline. O resultado não depende de
...
# --- todo vídeo em assets/videos/ vira data URI (mesmo esquema das imagens) ---
MIME_VIDEO_POR_EXTENSAO = {".mp4": "video/mp4", ".webm": "video/webm"}
for caminho_video in sorted(set(re.findall(r'src="(assets/videos/[^"]+)"', html))):
    extensao = pathlib.Path(caminho_video).suffix.lower()
    mime = MIME_VIDEO_POR_EXTENSAO.get(extensao)
    if not mime:
        sys.exit(f"Extensão de vídeo sem MIME mapeado: {caminho_video}")
    html = html.replace(f'src="{caminho_video}"', f'src="{data_uri(caminho_video, mime)}"')
...
html = html.replace(
    ...
    '<script src="assets/vendor/scene3d.min.js"></script>',
    libs + "<script>\n" + proteger(js) + "\n</script>\n"
    + "<script>" + proteger(ler("assets/vendor/scene3d.min.js")) + "</script>",
)
